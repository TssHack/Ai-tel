...
async def chat_with_ai(query, user_id):
    try:
        ...
        data = {
            "prompt": query,
            "userId": str(user_id),  # ارسال user_id به جای chat_id
            "network": True,
            "system": "",
...
async def handle_message(event):
    chat_id = event.chat_id  # شناسه گروه یا چت
    user_id = event.sender_id  # شناسه کاربر ارسال‌کننده پیام
    message = event.raw_text.strip().lower()

    # فقط در گروه‌ها کار کند، در چت خصوصی غیرفعال باشد
    if event.is_private:
        return
    ...
    if not any(word in message for word in ["ai", "pro", "gpt"]):
        return
    ...
    async with client.action(chat_id, "typing"):
        if api_type == "pro":
            ...
            response = await chat_with_ai(query, user_id)  # ارسال user_id به جای chat_id
